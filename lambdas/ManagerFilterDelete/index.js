...
exports.handler = async (event, context) => {
    ...
    if (event.pathParameters?.filterId) {
        ...
        if (jsonBody?.pid) {
            ...
            // failure log
            await createLog(context, 'フィルター', '削除', '失敗', '400', event.requestContext.identity.sourceIp, projectId, logAccountId, logData);
            return {
                statusCode: 400,
                headers: {
        ...
        if (event?.requestContext?.authorizer?.pid) {
            ...
            if (!projectId || validProjectId.indexOf(Number(projectId)) == -1) {
                // failure log
                await createLog(context, 'フィルター', '削除', '失敗', '403', event.requestContext.identity.sourceIp, projectId, logAccountId, logData);
                return {
                    statusCode: 403,
                    headers: {
                        'Access-Control-Allow-Origin': '*',
        ...
        try {
            ...
            logData[2] = {};
            logData[2].fieldName = "フィルター説明";
            logData[2].beforeValue = beforeResult[0].filterOverview;
            logData[2].afterValue = "";
            logData[3] = {};
            logData[3].fieldName = "フィルター設定";
            logData[3].beforeValue = beforeResult[0].filterQuery;
            ...
            if (!countFlag) {
                ...
                let [query_result] = await mysql_con.query(sql_data, [filterId]);
            ...
            // SubItem
            if (!countFlag) {
                let sql_data = `SELECT COUNT(eventSubItemId) AS count FROM EventSubItem WHERE filterId = ?`;
                let [query_result] = await mysql_con.query(sql_data, [filterId]);
                if (query_result[0].count >= 1) {
                    countFlag = true;
                }
            ...
            // SubCounselor
            if (!countFlag) {
                let sql_data = `SELECT COUNT(eventSubCounselorId) AS count FROM EventSubCounselor WHERE filterId = ?`;
                let [query_result] = await mysql_con.query(sql_data, [filterId]);
                if (query_result[0].count >= 1) {
                    countFlag = true;
                }
            ...
            if (countFlag) {
                ...
                return {
                    ...
                    body: JSON.stringify({
                        message: "filter are used in events.",
                        errorCode: 104
                    }),
                };
            }
            ...
            // フィルター削除
            let sql_data2 = `DELETE from Filter WHERE filterId = ?`;
            var [query_result2] = await mysql_con.query(sql_data2, [filterId]);

            await mysql_con.commit();
            // success log
            await createLog(context, 'フィルター', '削除', '成功', '200', event.requestContext.identity.sourceIp, projectId, logAccountId, logData);
            return {
                statusCode: 200,
                headers: {
                    'Access-Control-Allow-Origin': '*',
