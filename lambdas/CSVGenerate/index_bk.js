...
exports.handler = async (event) => {
    ...
    try {
        ...
        // console.log("query_result2 === ", query_result2);
        ...
        console.log('csvExportTemplateColumn', csvExportTemplateColumn);
        ...
        if (csvExportTemplateColumn) {
            ...
            let numOfField = 1;
            csvExportTemplateColumn.forEach(column => {
        ...
        let fieldColumns = '';
        let fieldFrom = '';
        let fieldColumnSQL = `SELECT * FROM Field WHERE projectId = ? AND fieldType IN (0, 1, 2, 3, 4, 5, 6, 7)`;
        let [fieldColumnSQLData] = await mysql_con.query(fieldColumnSQL, [projectId]);
        ...
        */
        ...
        count(Reservation.updatedAt) AS count
        FROM Reservation
        LEFT OUTER JOIN Customer ON Reservation.customerId = Customer.customerId
        INNER JOIN Event ON Customer.eventId = Event.eventId
        ${fromQuery}
        ${whereQuery}
        ORDER BY Reservation.updatedAt DESC`;
        ...
        if (recordCount >= 1) {
            ...
            */
            ...
            for (let i = 0; i < maxCount; i++) {
                ...
                if (i == 0) {
                    // 新  全てのユーザーデータを持ってくる
                    let sql_data = `SELECT Customer.*, Reservation.*, 
                            ${fieldColumns2[i]}
                            FROM Reservation USE INDEX FOR JOIN (index_customer_id)
                            LEFT OUTER JOIN Customer ON Reservation.customerId = Customer.customerId
                            INNER JOIN Event ON Customer.eventId = Event.eventId
                    ${fieldFrom2[i]}
                    ${fromQuery}
                    ...
                    ORDER BY 
                        ...
                        `;
                    // ORDER BY Reservation.updatedAt DESC`;
                    // console.log(`sql_data${i}`, sql_data);
                    // query data
                    [query_result_array[i]] = await mysql_con.query(sql_data, [projectId]);
            ...
            // 新
            let customerIds = query_result_array[0].map((row) => {
                return row.customerId;
            });
            let sql_data2 = `SELECT * FROM CustomerField WHERE customerId IN (?)`;
            console.log('sql_data2', sql_data2);
            console.log('customerIds', customerIds);
            // query data
            [query_result2] = await mysql_con.query(sql_data2, [customerIds]);
        ...
        const textFormat = (value, formatMethod) => {
        ...
        const prepareListType = (fieldId, val, formatMethod) => {
        ...
        // Get query count
        const numberOfRows = query_count_result[0].count;
        // console.log(numberToDivideLines);
        console.log('my checking >>>>>>>>>>>>>>', numberOfRows);
        ...
        if (numberOfRows !== 0 && query_result) {
                    ...
                    if (typeof record !== undefined && record !== undefined) {
                        ...
                        for (var k = 0; k < csvExportTemplateColumn.length; k++) {
                            ...
                            if (targetColumn?.displayControl === undefined || targetColumn?.displayControl == '0') {
                                // 値が存在しない
                                if (val === undefined) {
                                    let row = query_result2.filter((row) => {
                                        // if (row.customerId == "153806") {
                                        //     console.log("row", row);
                                        // }
                                        if (row.customerId == record.customerId && row.fieldId == targetColumn.fTypeId) {
                                            return row;
                                        }
                                    });
                                    ...
                                    if (row) {
                                        if (row.length >= 1) {
                                            switch (targetColumn.inputBox.type) {
                                                ...
                                                case 1:
                                                case 2:
                                                    val = row[0].customerFieldText;
                                                    break;
                                                case 3:
                                                    val = row[0].customerFieldList;
                                                    break;
                                                case 4:
                                                    val = row[0].customerFieldBoolean;
                                                    break;
                                                case 5:
                                                case 6:
                                                case 7:
                                                    val = row[0].customerFieldInt;
                                                    break;
                                            }
                                        }
                                        // if (record.customerId == "176") {
                                ...
                                switch (targetColumn.inputBox.type) {
                                    ...
                                    case 3:
                                        let val2 = "";
                                        if (val !== undefined && val !== null && fieldQueryListData !== undefined && fieldQueryListData !== null) {
                                            for (let l = 0; l < fieldQueryListData.length; l++) {
                                                ...
                                                for (let m = 0; m < fieldStyleLookup.length; m++) {
                                                    ...
                                                    for (let n = 0; n < val.length; n++) {
                                                        if (fieldStyleLookup[m].fieldListCode == val[n].id) {
                                                            if (val2 == "") {
                                            ...
                                            // value exist
                                            // if (targetColumn.fTypeId && val && targetColumn.fieldFormat) {
                                            //     val = '"'+prepareListType(targetColumn.fTypeId, val, targetColumn.fieldFormat)+'"';
                                            //     // console.log('record list type', val);
                                            // }
                                            // if (record.customerId == "168149") {
                            ...
                            // 条件にマッチした値を出力
                                ...
                                //     console.log("targetColumn.innerDrag 0", JSON.stringify(targetColumn));
                                // }
                                let addFlag = false;
                                ...
                                // if (record.customerId == "69363" && targetColumn.fTypeId == 2075) {
                                //     console.log("targetColumnOrder", targetColumnOrder);
                                // }
                                ...
                                for (let x = 0; x < targetColumnOrder.length; x++) {
                                    ...
                                    for (let y = 0; y < filterQueryInfo.length; y++) {
                                        ...
                                        if (targetColumnOrder[x].fTypeId == filterQueryInfo[y].filterId) {
                                            ...
                                            // }
                                            if (commonFunctions2.getUserDataFromFilter(filterQueryInfo[y].filterQuery, record, fieldInfo)) {
                                                // if (record.customerId == "69363" && targetColumn.fTypeId == 2075) {
        ...
        if (password && password !== '') {
            ...
            archive = archiver('zip-encrypted', { zlib: { level: 9 }, encryptionMethod: 'aes256', password: password, forceLocalTime: true });
        }
        else {
            // create archive
            archive = archiver('zip', { zlib: { level: 9 }, forceLocalTime: true });
        }
        // Upload process to S3 bucket
        /*
                const uploadFromStream = (s3, path) => {
        ...
        */
        // Pipe!
        // archive.pipe(uploadFromStream(s3, filePath));
        ...
        tmpFiles.map((file, index) => {
            archive.append(fs.createReadStream(file), { name: `${csvFileName}_${(index + 1)}.csv` });
        });
        ...
        await uploadFromStream2(uploadStream, filePath)
        ...
        let response = { records: query_result2[0] };
