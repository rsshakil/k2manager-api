...
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);`;
        ...
        if (csvExportTemplateColumn) {
            ...
            [query_field_result] = await mysql_con.query(sql_field, [projectId]);
        ...
        for (let i = 0; i < fieldColumnSQLData.length; i++) {
            customerFieldIds.push(fieldColumnSQLData[i].fieldId);
        ...
        */
        ...
        if (recordCount >= 1) {
            ...
            */
            ...
            let maxCount = Math.ceil(fieldColumnSQLData.length / tableLimit);
            ...
            let index = 1;
            while (recordCount > offset && offset < 50000) {
                ...
                if (query_result_array[0].length > 0) {
                    const temFile = await generateTempFiles(index, query_result_array, customerFieldIds, maxCount, headerData, numberToDivideLines, csvExportTemplateColumn, itemAliasArr, filterQueryInfo, fieldInfo, encodePattern);
                    tmpFiles.push(temFile);
                }

                offset += query_result_array[0].length;
                index++;
    ...
    async function generateTempFiles(index = 1, query_result_array = [], CustomerFieldIds = [], maxCount, headerData, numberToDivideLines, csvExportTemplateColumn, itemAliasArr, filterQueryInfo, fieldInfo, encodePattern) {
        let query_result = [];
        let query_result2 = [];
        // 新
        let customerIds = query_result_array[0].map(x => x.customerId);

        let sql_data2 = `SELECT customerId, fieldId, customerFieldText, customerFieldList, customerFieldBoolean, customerFieldInt FROM CustomerField WHERE customerId IN (?) AND fieldId IN (?)`;
        // console.log('sql_data2', sql_data2);
        // console.log('customerIds', customerIds);
        // query data
        [query_result2] = await mysql_con.query(sql_data2, [customerIds, CustomerFieldIds]);
        ...
        if (numberOfRows !== 0 && query_result.length > 0) {
            ...
            for (let i = 1; i <= 1; i += 1) {
                ...
                for (let j = tempRow; j < numberOfRows && j < (i * numberToDivideLines); j++) {
                    ++tempRow;
                    let record = query_result[j];   //console.log('my records --------------------------->>>>>>>>', record)
