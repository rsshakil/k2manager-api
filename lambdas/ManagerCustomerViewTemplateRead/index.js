...
/**
 * ManagerCustomerViewTemplateRead.
 * 
 * @param {*} event 
...
exports.handler = async (event) => {
    ...
    if (event.pathParameters && event.pathParameters?.customerTemplateViewId) {
...
z
        let customerTemplateViewId = event.pathParameters.customerTemplateViewId;
        let parameter = [];
        // get one record sql
        // const sql_data = `SELECT * FROM CustomerViewTemplate WHERE CustomerViewTemplate.customerTemplateViewId = ? AND CustomerViewTemplate.projectId = ?`
        const sql_data = `SELECT CustomerViewTemplate.*, Domain.domainURL AS appURL, App.appId FROM CustomerViewTemplate 
        INNER JOIN App ON CustomerViewTemplate.appId = App.appId
        INNER JOIN Domain ON App.appDomainId = Domain.domainId
        WHERE 1=1 AND CustomerViewTemplate.projectId = ?`
        parameter.push(Number(customerTemplateViewId));
...
console.log("event?.requestContext?.authorizer?.rid = ", event?.requestContext?.authorizer?.rid);
        ...
        if (event?.requestContext?.authorizer?.rid) {
            whereQuery = `AND (
                JSON_CONTAINS(customerViewTemplateAuthRole, '?', '$' ) 
                    OR (
                        customerViewTemplateAuthRole IS NULL 
                        OR
                        JSON_EXTRACT(customerViewTemplateAuthRole, '$') = JSON_ARRAY()
                    )
                )
            `;
        ...
        // get list sql
        // const sql_data = `SELECT * FROM CustomerViewTemplate WHERE 1=1 ${whereQuery} AND CustomerViewTemplate.projectId = ? ORDER BY CustomerViewTemplate.updatedAt DESC LIMIT ?, ?`
