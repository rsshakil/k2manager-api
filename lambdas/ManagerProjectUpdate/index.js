    ...
    if (event.pathParameters?.projectId) {
        ...
        if (event?.requestContext?.authorizer?.pid) {
            ...
            if (validProjectId.indexOf(Number(event.pathParameters?.projectId)) == -1) {
                // failure log
                await createLog(context, 'プロジェクト', '更新', '失敗', '403', event.requestContext.identity.sourceIp, projectId, logAccountId, logData);
                return {
                    statusCode: 403,
                    headers: {
        ...
        try {
            ...
            if (beforeResult.length === 0) {
                ...
                // failure log
                await createLog(context, 'プロジェクト', '更新', '失敗', '400', event.requestContext.identity.sourceIp, projectId, logAccountId, logData);
                return {
                    statusCode: 400,
                    headers: {
                        "Access-Control-Allow-Origin": "*",
            ...
            logData[1].beforeValue = beforeResult[0].projectName;
            ...
            logData[4].beforeValue = beforeResult[0].projectCsvCharacterCode;
            logData[4].afterValue = projectCsvCharacterCode;

            const updatedAt = Math.floor(new Date().getTime() / 1000);
            let sql_data = `UPDATE Project SET projectName = ?, projectStatus = ?, projectCsvCharacterCode = ?, memo = ?, updatedAt = ?, updatedBy = ? WHERE projectId = ?;`;
            let sql_param = [
                projectName,
                projectStatus,
                projectCsvCharacterCode,
            ...
            //     await mysql_con.rollback();
            //     // failure log
            //     await createLog(context, 'プロジェクト', '更新', '失敗', '400', event.requestContext.identity.sourceIp, logAccountId, logData);
            //     return {
            //         statusCode: 400,
            //         headers: {
            //             "Access-Control-Allow-Origin": "*",
            ...
            // もしprojectStatusが0だった場合配下のAppも停止（メンテナンスに）する
            if (projectStatus == 0) {
                const sql_app = `UPDATE App 
                    LEFT OUTER JOIN Event ON App.eventId = Event.eventId
                    LEFT OUTER JOIN Project ON Event.projectId = Project.projectId
                    SET appStatus = 0
                    WHERE Project.projectId = ?;`;
                let [query_result2] = await mysql_con.execute(sql_app, [projectId]);
            }
            ...
            // success log
            await createLog(context, 'プロジェクト', '更新', '成功', '200', event.requestContext.identity.sourceIp, projectId, logAccountId, logData);
            return {
                statusCode: 200,
                headers: {
                    "Access-Control-Allow-Origin": "*",
        ...
        // failure log
        await createLog(context, 'プロジェクト', '更新', '失敗', '400', event.requestContext.identity.sourceIp, null, logAccountId, logData);
        return {
            statusCode: 400,
            headers: {
                "Access-Control-Allow-Origin": "*",
