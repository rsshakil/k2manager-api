...
exports.handler = async (event) => {
    ...
    if (event.pathParameters && event.pathParameters?.projectId) {
        ...
        console.log("event.requestContext.authorizer.authRole", event?.requestContext?.authorizer?.pid);
        ...
        let parameter = [];
        // get one record sql
        const sql_data = `SELECT * FROM Project WHERE Project.projectId = ?`
...
console.log("project一覧取得メソッド");
        ...
        // Other search query
        // projectName
        if (jsonBody.projectName) {
            let projectName = jsonBody.projectName;
            if (projectName.slice(0, 1) != '*' && projectName.slice(-1) != '*') {
                whereQuery += ` AND Project.projectName = ?`;
                parameter.push(projectName);
            } else {
                whereQuery += ` AND Project.projectName like ?`;
                parameter.push(projectName.replace(/(^\*)|(\*$)/g, '%'));
            }
        }
        if (validProjectId) {
            whereQuery += ` AND Project.projectId IN (?)`
            parameter.push(validProjectId);
        }
        // limit
        ...
        // total count sql
        const sql_count = `SELECT COUNT(projectId) AS count FROM Project WHERE 1=1 ${whereQuery}`;
        // get list sql
        const sql_data = `SELECT * FROM Project WHERE 1=1 ${whereQuery} ORDER BY updatedAt DESC LIMIT ?, ?`;
