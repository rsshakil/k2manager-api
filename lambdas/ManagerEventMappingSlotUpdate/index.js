...
exports.handler = async (event, context) => {
    ...
    try {
        ...
        if (rowsData != undefined) {
            // beforeDataの作成
            let beforeSql = `SELECT slotId, datetime, itemId, itemSubId, counselorId, counselorSubId, sort, maxReservationCount, reservationCount, commonReservationCount FROM EventSlot WHERE mappingId = ?`;
            ...
            for (let i = 0; i < eventItemIdArr.length; i++) {
                rowsData.forEach(item => {
                    ...
                    if (!eventItemIdArr[i].includes("_common") && !eventItemIdArr[i].includes("_limitflag") && !eventItemIdArr[i].includes("_reservation") && !eventItemIdArr[i].includes("_remain") && !eventItemIdArr[i].includes("_slotId")) {
                        ...
                        if (eventInstituteItemType == 1) updatedValue.counselorId = rowItemId[0] != 'null' ? Number(rowItemId[0]) : 0;
                        else updatedValue.itemId = rowItemId[0] != 'null' ? Number(rowItemId[0]) : 0;
            ...
            for (let i = 0; i < updatedValueArr.length; i++) {
                ...
                if (eventInstituteItemType == 1) { //for counselor
                    sql_query1 = `UPDATE EventSlot SET  maxReservationCount = ?, updatedBy = ?, updatedAt = ? WHERE mappingId = ? AND datetime = ? AND counselorId = ? AND commonReservationCount <= ?`;
                    parameter = [updatedValueArr[i].maxReservationCount, updatedBy, updatedAt, mappingId, updatedValueArr[i].datetime, updatedValueArr[i].counselorId, updatedValueArr[i].maxReservationCount];
                } else { //for item
                    sql_query1 = `UPDATE EventSlot SET  maxReservationCount = ?, updatedBy = ?, updatedAt = ? WHERE mappingId = ? AND datetime = ? AND itemId = ? AND commonReservationCount <= ?`;
                    parameter = [updatedValueArr[i].maxReservationCount, updatedBy, updatedAt, mappingId, updatedValueArr[i].datetime, updatedValueArr[i].itemId, updatedValueArr[i].maxReservationCount];
                }
        ...
        // afterData
        let afterSql = `SELECT slotId, datetime, itemId, itemSubId, counselorId, counselorSubId, sort, maxReservationCount, reservationCount, commonReservationCount FROM EventSlot WHERE mappingId = ?`;
...
async function createLog(context, _target, _type, _result, _code, ipAddress, accountId, logData = null) {
    try {
        ...
        console.log('Log create error: ', err)
    }
}