...
/**
 * ManagerCustomerViewTemplateCreate.
 * 
 * @param {*} event 
...
exports.handler = async (event, context) => {
    ...
    try {
        let validProjectId;
        console.log('authorization queryStringParameters', event.queryStringParameters?.pid);
        console.log('authorization requestContext', JSON.parse(event?.requestContext?.authorizer?.pid));
        if (event?.requestContext?.authorizer?.pid) {
            ...
            if (!event.queryStringParameters?.pid || validProjectId.indexOf(Number(event.queryStringParameters?.pid)) == -1) {
                ...
                await createLog(context, '顧客一覧テンプレート', '作成', '失敗', '403', event.requestContext.identity.sourceIp, projectId, logAccountId, logData);
                return {
                    statusCode: 400,
                    headers: {
        ...
        if (event.pathParameters?.customerViewTemplateId) {
            ...
            const sql_data = `SELECT * FROM CustomerViewTemplate WHERE CustomerViewTemplate.customerViewTemplateId = ? AND CustomerViewTemplate.projectId = ?`;
            parameter.push(Number(customerViewTemplateId));
            ...
            if (query_result && query_result[0]) {
                ...
                // insert data query
                let copy_sql = `INSERT INTO CustomerViewTemplate (
                    projectId,
                    customerViewTemplateName,
                    customerViewTemplateSearch,
                    customerViewTemplateColumn,
                ...
                let newCustomerViewTemplateId = query_copy_result.insertId;
                ...
                logData[1] = {};
                logData[1].fieldName = "顧客一覧テンプレートID";
                logData[1].beforeValue = customerViewTemplateId;
                logData[1].afterValue = newCustomerViewTemplateId;
                ...
                // success log
                await createLog(context, '顧客一覧テンプレート', '複製', '成功', '200', event.requestContext.identity.sourceIp, projectId, logAccountId, logData);
                return {
                    statusCode: 200,
                    headers: {
                        "Access-Control-Allow-Origin": "*",
                ...
                // failure log
                await createLog(context, '顧客一覧テンプレート', '複製', '失敗', '400', event.requestContext.identity.sourceIp, projectId, logAccountId, logData);
                return {
                    statusCode: 400,
                    headers: {
                        "Access-Control-Allow-Origin": "*",
                    ...
                    body: JSON.stringify("{message: invalid customerViewTemplateId}"),
                };
            }
        }
        // 顧客一覧テンプレートの新規作成
            ...
            // insert data query
            let sql_data = `INSERT INTO CustomerViewTemplate (
                projectId,
                customerViewTemplateName,
                customerViewTemplateSearch,
                customerViewTemplateColumn,
            ...
            if (query_result.length === 0) {
                ...
                // failure log
                await createLog(context, '顧客一覧テンプレート', '作成', '失敗', '400', event.requestContext.identity.sourceIp, projectId, logAccountId, logData);
                return {
                    statusCode: 400,
                    headers: {
                        "Access-Control-Allow-Origin": "*",
            ...
            // console.log("response:", response);
            // success log
            await createLog(context, '顧客一覧テンプレート', '作成', '成功', '200', event.requestContext.identity.sourceIp, projectId, logAccountId, logData);
            return {
                statusCode: 200,
                headers: {
                    "Access-Control-Allow-Origin": "*",
