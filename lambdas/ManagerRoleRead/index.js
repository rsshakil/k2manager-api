...
exports.handler = async (event) => {
    ...
    if (event.pathParameters && event.pathParameters?.roleId) {
        let roleId = event.pathParameters.roleId;
        let parameter = [];
        // get one record sql
        const sql_data = `SELECT 
                Role.*,
                acnt.numberOfAccounts
            FROM Role 
                LEFT OUTER JOIN (SELECT roleId, COUNT(id) AS numberOfAccounts FROM Account GROUP BY roleId) AS acnt ON Role.roleId = acnt.roleId
            WHERE
                Role.roleId = ? LIMIT 0, 1`
        parameter.push(Number(roleId));
    ...
    // Get record list
        ...
        // total count sql
        const sql_count = `SELECT COUNT(roleId) AS count FROM Role`;
        // get list sql
        const sql_data = `SELECT 
                Role.*,
                roleRelationStyle,
