...
exports.handler = async (event, context) => {
    ...
    try {
        ...
        const project_sql = `SELECT * FROM Project WHERE projectId = ? LIMIT 1`;
        const [query_project_result] = await mysql_con.execute(project_sql, [projectId]);

        let encodePattern = 0;
        if (query_project_result.length > 0) encodePattern = query_project_result[0].projectCsvCharacterCode;
        ...
        console.log('my cehcking >>>>>>>>>>>>>>>', query_result);


        //  const encodePattern = query_result[0].projectCsvCharacterCode;
        ...
        //Set encode pattern accrding to project setting
        let writeData = header.join(',') + '\n'; //encodePattern = 1 (without BOM)

        if (encodePattern == 0) { // add BOM (byte order mark)
            writeData = '\ufeff' + writeData;
        }
        else if (encodePattern == 2) {    // convert to Shift-JIS
            // Convert string to code array
            const unicodeArray = encoding.stringToCode(writeData);
            const sjisArray = encoding.convert(unicodeArray, { to: 'SJIS', from: 'UNICODE' });

            writeData = Buffer.from(sjisArray);
        }
